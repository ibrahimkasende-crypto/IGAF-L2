...
let rscHandler = null;
let nodeToWebRequest = null;
let sendWebResponse = null;
...
const server = createServer((req, res) => {
  Promise.resolve()
    .then(async () => {
      ...
      if (!rscHandler || !nodeToWebRequest || !sendWebResponse) {
...
import("vinext/server/prod-server")
  .then(async (vinext) => {
    nodeToWebRequest = vinext.nodeToWebRequest;
    sendWebResponse = vinext.sendWebResponse;
